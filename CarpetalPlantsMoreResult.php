...
<html lang="en">
<head>
  <title>Carpetal Plants More Info</title>
  <meta charset="utf-8">
...
<header>
  ...
  <div class="container">
    ...
    if (isset($_GET['name'])) {
        $selectedPlantName = $_GET['name'];

        // Query to retrieve fish details for the selected fish name
        $query = "SELECT * FROM plants WHERE Plant_Name = '" . mysqli_real_escape_string($con, $selectedPlantName) . "'";
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            echo '<tr>';
            echo '<th class="plant-image">Plant Image</th>';
            echo '<td>';
            if ($row['Image']) {
                echo '<div><img src="data:image/jpeg;base64,' . base64_encode($row['Image']) . '" alt="Plant Image" class="plant-image"></div>';
            } else {
                echo '<div><img src="path/to/placeholder-image.jpg" alt="No Image" class="plant-image"></div>';
            }
            echo '</td>';
            echo '</tr>';
            // Display other plants details
            echo '<tr>';
            echo '<th>Hardiness</th>';
            echo '<td>' . $row['Hardiness'] . '</td>';
            echo '</tr>';
            echo '<tr>';
            echo '<th>Lighting Requirement</th>';
            echo '<td>' . $row['Lighting_Requirement'] . '</td>';
            echo '</tr>';
            echo '<tr>';
            echo '<th>Category</th>';
            ...
            echo '</tr>';
            echo '<tr>';
            echo '<th>Aquascape Placement</th>';
            echo '<td>' . $row['Aquascape_Placement'] . '</td>';
            echo '</tr>';
            echo '<tr>';
            echo '<th>Plant Type</th>';
            echo '<td>' . $row['Plant_Type'] . '</td>';
            echo '</tr>';
            echo '<tr>';
            echo '<th>pH Level</th>';
            ...
            echo '<tr>';
            echo '<th>Region</th>';
           echo '<td style="white-space: pre-wrap;">' . $row['Region'] . '</td>';
           echo '</tr>';
            echo '</table>';
        } else {
